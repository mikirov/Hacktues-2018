...
from config.game_config import PLAYER_FRAME_CHANGE

class Animation:
    def __init__(self, owner, width, height, max_frames, frame=0,
                 speed=PLAYER_FRAME_CHANGE):
        self.owner = owner
    ...
    def stop(self, screen):
        current_frame = pygame.Rect(
            int(self.starting_frame) * self.width, self.height * self.owner.current_facing.value,
            self.width, self.height
        )
        screen.blit(self.owner.image, (self.owner.x, self.owner.y), current_frame) 
